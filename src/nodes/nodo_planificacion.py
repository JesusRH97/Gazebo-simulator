...
MIN_T = 8.85  # min prediction time [m]
...
def callback(data):
    ...
    x1 = x_prius + (5.0*math.cos(yaw))
    y1 = y_prius + (5.0*math.sin(yaw))

    x2 = x_prius - (5.0*math.cos(yaw))
    y2 = y_prius - (5.0*math.sin(yaw))

    x3 = x_prius - (8.0*math.cos(yaw))
    y3 = y_prius - (8.0*math.sin(yaw))

    x4 = x_prius + (10.0*math.cos(yaw))
    y4 = y_prius + (10.0*math.sin(yaw))

    x5 = x_prius + (15.0*math.cos(yaw))
    y5 = y_prius + (15.0*math.sin(yaw))

    x6 = x_prius + (20.0*math.cos(yaw))
    y6 = y_prius + (20.0*math.sin(yaw))
...
def main():
    ...
    rospy.init_node("planificacion", anonymous=False)
    ...
    s0 = 420.0  # current course position
    ...
    for i in range(SIM_LOOP):
        ...
        try:
            ...
            nodo.pose.position.x = path.x[1] + (2.5*math.sin(angle - 1.57))
            nodo.pose.position.y = path.y[1] - (2.5*math.sin(angle - 1.57))
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(SIM_LOOP):
            ...
            if show_animation:  # pragma: no cover
                ...
                plt.xlim(path.x[8] - area, path.x[1] + area)
                plt.ylim(path.y[8] - area, path.y[1] + area)
